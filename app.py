...
def load_statistics() -> pd.DataFrame:
    ...
    # Strip any stray whitespace or quotes from column names
...
# Map of possible alternative column names from the API
_COL_ALIASES = {
...
def merge_data(gdf: gpd.GeoDataFrame, df: pd.DataFrame, year: int, sugu: str) -> gpd.GeoDataFrame:
    ...
    subset = df[(df[col_aasta] == year) & (df[col_sugu] == sugu)].copy()
...
with st.expander("🔍 Silumine (veaotsing)", expanded=False):
...
try:
    ...
    st.info("Ava 'Silumine' sektsiooni ülal, et näha tegelikke veerunimesid.")
    st.stop()
...
if show_table:
    ...
    st.dataframe(table, use_container_width=True, height=460)import requests
import pandas as pd
from io import StringIO
import json
import geopandas as gpd
